...
class BinaryTree {
  ...
  maxSum() {
    ...
    function findMaxSum(node) {
      if (node === null) return 0;
      let left = Math.max(findMaxSum(node.left), 0); // Ignore paths with negative sum
      let right = Math.max(findMaxSum(node.right), 0); // Ignore paths with negative sum

      // Update maxPathSum if the path through the current node is greater
